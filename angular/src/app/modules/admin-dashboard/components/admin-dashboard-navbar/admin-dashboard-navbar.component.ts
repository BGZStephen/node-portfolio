...
export class AdminDashboardNavbarComponent implements OnInit {
  ...
  menuStyle() {
    if (!this.menuVisible) {
      ...
      const minHeight = document.getElementsByClassName('navbar-list')[0].children.length * 53;
  ...
  subMenuStyle(index: number) {
    if (this.activeSubMenu === index) {
      const minHeight = document.getElementsByClassName('navbar-secondary-list')[index].children.length * 51;
